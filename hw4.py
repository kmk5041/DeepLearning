...
class nn_convolutional_layer:
    ...
    # Extract dimensions
        ...
        out_channels, _, filter_height, filter_width = self.W.shape

    # Use F.unfold to create sliding windows
        x_unfolded = F.unfold(x, kernel_size=(filter_height, filter_width))

    # Reshape weights for matrix multiplication
        W_reshaped = self.W.view(out_channels, -1)

    # Perform convolution using matrix multiplication
        out_unfolded = torch.matmul(W_reshaped, x_unfolded)

    # Reshape the output to match the expected output dimensions
        out_height = height - filter_height + 1
        out_width = width - filter_width + 1
        out = out_unfolded.view(batch_size, out_channels, out_height, out_width)
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        x_unfolded = x.unfold(2, self.pool_size, self.stride).unfold(3, self.pool_size, self.stride)

        # x_unfolded shape: (batch_size, channels, out_height, out_width, pool_size, pool_size)

        # Reshape to (batch_size, channels, out_height, out_width, pool_size * pool_size)
        x_unfolded = x_unfolded.contiguous().view(batch_size, channels, out_height, out_width, -1)

        # Take the max over the last dimension
        out, _ = x_unfolded.max(dim=-1)
